...
public class HardStoneBlock extends TaskmodModElements.ModElement {
	@ObjectHolder("taskmod:hard_stone")
	public static final Block block = null;
	public HardStoneBlock(TaskmodModElements instance) {
		super(instance, 1);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("hard_stone", "hard_stone", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 16)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(10, 0, 0, 125))));
		}
	}
}
